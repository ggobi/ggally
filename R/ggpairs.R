...
ggpairs <- function(
  ...
  # Cas où il n'y a pas de colonne cible
  if (is.null(target_column)) {
    return(ggpairs(data, columns = columns, title = title, upper = upper, lower = lower, diag = diag))
  }
  ...
  if (num_vars == 0) {
    stop("Il n'y a pas d'autres variables à comparer avec la colonne cible.")
  }

  full_plot <- ggpairs(data, columns = c(target_column, other_vars))

  # Extraire les graphiques de la première colonne (hors diagonale)
  extracted_plots <- lapply(seq_len(num_vars), function(i) {
    full_plot[i + 1, 1]
  })

  # Construire un ggmatrix avec une seule colonne
