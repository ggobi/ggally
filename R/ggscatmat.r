...
scatmat <- function(data, columns=1:ncol(data), color=NULL) {
  ...
  if (ncol(dn) == 0) {
    ...
    r <- ggplot(ltdata.new, mapping=aes(x=xvalue, y=yvalue)) +      
      theme(axis.title.x=element_blank(), axis.title.y=element_blank()) + 
      facet_grid(ylab ~ xlab, scale = "free") + 
...
ggscatmat <- function(data, columns=1:ncol(data), color=NULL){  
  ...
  if (is.null(color)){
    ...
    plot <- scatmat(data,column=columns, color=color) + 
      geom_text(data=a, aes(label=r, color=colorcolumn)) + labs(color = color)
  }
