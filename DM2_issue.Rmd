...
## Première Proposition Chatgpt

### Select only the 'released' column against all other variables 

```{r}
plot <- ggpairs(Arrests, columns = c(1, 2:5), mapping = ggplot2::aes(color = released)) + theme(axis.text.x = ggplot2::element_text(angle = 45, hjust = 1))
...
```{r}
plot <- ggpairs(Arrests, columns = c(1, 2:5), mapping = ggplot2::aes(color = released)) + theme(axis.text.x = ggplot2::element_text(angle = 45, hjust = 1), panel.grid.major = ggplot2::element_blank(), panel.grid.minor = ggplot2::element_blank(), legend.title = ggplot2::element_text(size = 12), legend.text = ggplot2::element_text(size = 10))
